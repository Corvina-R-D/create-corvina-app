...
function setImagePullPolicyToIfNotPresent() {
  kubectl patch deployment $1 --context corvina-minikube -n corvina-app-[| .Name |] --type='json' -p='[{"op": "replace", "path":"/spec/template/spec/containers/0/imagePullPolicy", "value": "IfNotPresent"}]';
}
function restartDeployment() {
  kubectl --context corvina-minikube -n corvina-app-[| .Name |] rollout restart deploy $1;
}
